...
namespace Lab_4_Threads
{
    public partial class Form1 : Form
    {
        ...
        private void EncryptionWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (e.Cancelled)
            {
                MessageBox.Show("Ïðîöåññ øèôðîâàíèÿ îòìåí¸í.", "Îòìåíà", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else if(e.Result != null)
            {
                ...
                string message = $"Øèôðóâàííÿ çàâåðøåíî!\n" +
                                 $"Íàçâà ôàéëà: {encryptedFileInfo.Name}\n" +
                                 $"Ðîçì³ð ôàéëà: {encryptedFileInfo.Length} áàéò\n" +
                                 $"×àñ, âèòðà÷åíèé íà øèôðóâàííÿ: {result.ElapsedTime.ToString(@"hh\:mm\:ss")}";
                MessageBox.Show(message, "Ðåçóëüòàòè øèôðóâàííÿ", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("Ïîìèëêà ïðè øèôðóâàíí³!", "Ïîìèëêà", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
